...
func activate() -> void:
	if state == State.IDLE:
		var direction = "down" if global_position.y >= upper_y - 0.05 else "up"
		NetworkManager.send_elevator_use("activate", direction)
...
func _on_elevator_used(action: String, floor_name: String):
	...
	if action == "activate":
		state = State.MOVING_DOWN if floor_name == "down" else State.MOVING_UP
	elif action == "call":
		state = State.MOVING_DOWN if floor_name == "lower" else State.MOVING_UP
...
func _physics_process(delta: float) -> void:
	...
	if state == State.MOVING_DOWN:
		...
		if abs(global_position.y - upper_y) < 0.01:
			...
			state = State.IDLE
			_finish_arrival("upper")

func _finish_arrival(floor_name: String) -> void:
	elevator_doors._open_doors()
	if floor_name == "upper":
		upper_doors._open_doors()
	else:
